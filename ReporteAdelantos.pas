...
type
  TfReporteAdelantos = class(TForm)
...
var
  fReporteAdelantos: TfReporteAdelantos;
  sFecha1: string;
  sFecha2: string;
...
procedure TfReporteAdelantos.FormCreate(Sender: TObject);
begin
...
procedure TfReporteAdelantos.oBtn_CloseClick(Sender: TObject);
begin
  exit;
end;

procedure TfReporteAdelantos.oBtn_DesignClick(Sender: TObject);
begin
  self.Print_Report(0);
end;

procedure TfReporteAdelantos.oBtn_PDFClick(Sender: TObject);
begin
  bPrintToFile := true;
  self.Print_Report(4);
...
procedure TfReporteAdelantos.oBtn_PreviewClick(Sender: TObject);
begin
...
procedure TfReporteAdelantos.oBtn_PreviewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if bSwitchPrev = true then
    self.Print_Report(1)
  else
...
procedure TfReporteAdelantos.oBtn_PrintClick(Sender: TObject);
begin
  self.oBtn_Preview.Enabled := false;
  self.oBtn_Print.Enabled := false;
...
procedure TfReporteAdelantos.Print_Report(iOptcion: integer);
var
  sSql2: string;
  sCodCte: string;
  sHora1: string;
...
begin
  ...
  sSql2 := sSql2 + 'SELECT * ';
  sSql2 := sSql2 + 'FROM adelantos ';
  sSql2 := sSql2 + 'WHERE 1=1 ';
  sSql2 := sSql2 + 'AND ( Fecha >= "' + sFecha1 + '" AND Fecha <= "' + sFecha2 + '") ';
...
procedure TfReporteAdelantos.oBtn_SalirClick(Sender: TObject);
begin
  close;
end;

procedure TfReporteAdelantos.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = VK_F11 then
  begin
    if bSwitchPrev = true then
...
procedure TfReporteAdelantos.FormShow(Sender: TObject);
begin
  bSwitchPrev := true;
end;

procedure TfReporteAdelantos.oFecha01Exit(Sender: TObject);
begin
  self.oFecha1.date := self.oFecha01.Value;
end;

procedure TfReporteAdelantos.oFecha02Exit(Sender: TObject);
begin
...
procedure TfReporteAdelantos.Make_Reporte(sSqlFac: string; iOptcion: integer);
var
  sFileRep, cPath: string;
begin
  sFileRep := utilesV20.sPathReports + '\Rep_Adelantos1.fr3';
  ...
  Reportes.Queries[1].DB_DSName := 'oQryAdelantos';
  {
