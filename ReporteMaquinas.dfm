object fReporteMaquinas: TfReporteMaquinas
  Left = 0
  Top = 0
  BorderIcons = []
  Caption = 'Reportes general de m'#225'quinas'
  ClientHeight = 518
  ClientWidth = 542
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnActivate = FormActivate
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label3: TLabel
    Left = 19
    Top = 16
    Width = 59
    Height = 16
    Caption = 'Empresa:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    Transparent = True
  end
  object Shape2: TShape
    Left = -23
    Top = 63
    Width = 570
    Height = 2
    Pen.Color = clDefault
  end
  object olFecha1: TLabel
    Left = 23
    Top = 145
    Width = 40
    Height = 16
    Caption = 'Desde:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object olFecha2: TLabel
    Left = 321
    Top = 144
    Width = 37
    Height = 16
    Caption = 'Hasta:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object olCliente: TLabel
    Left = 23
    Top = 221
    Width = 44
    Height = 16
    Caption = 'Cliente:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object olModelos: TLabel
    Left = 34
    Top = 288
    Width = 33
    Height = 16
    Caption = 'Juego'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Shape1: TShape
    Left = -12
    Top = 203
    Width = 570
    Height = 2
    Pen.Color = clDefault
  end
  object Label1: TLabel
    Left = 23
    Top = 125
    Width = 109
    Height = 16
    Caption = 'Fecha de creasi'#243'n:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Shape4: TShape
    Left = -23
    Top = 421
    Width = 570
    Height = 2
    Pen.Color = clDefault
  end
  object Shape5: TShape
    Left = -16
    Top = 115
    Width = 570
    Height = 2
    Pen.Color = clDefault
  end
  object olRuta: TLabel
    Left = 30
    Top = 321
    Width = 37
    Height = 16
    Caption = 'Rutas:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Visible = False
  end
  object Label2: TLabel
    Left = 20
    Top = 355
    Width = 47
    Height = 16
    Caption = 'Chapas:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 8
    Top = 389
    Width = 59
    Height = 16
    Caption = 'M'#225'quinas:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label5: TLabel
    Left = 4
    Top = 254
    Width = 63
    Height = 16
    Caption = 'Consesion:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object oLst_emp: TDBLookupComboboxEh
    Left = 84
    Top = 8
    Width = 373
    Height = 30
    DynProps = <>
    DataField = ''
    EditButtons = <>
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Arial'
    Font.Style = []
    KeyField = 'emp_id'
    ListField = 'emp_descripcion'
    ListSource = oDs_Emp
    ParentFont = False
    TabOrder = 0
    Visible = True
    OnCloseUp = oLst_empCloseUp
    OnExit = oLst_empExit
  end
  object oFecha1: TDBDateTimeEditEh
    Left = 23
    Top = 164
    Width = 122
    Height = 24
    DynProps = <>
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    Kind = dtkDateEh
    ParentFont = False
    TabOrder = 4
    Visible = True
  end
  object oFecha2: TDBDateTimeEditEh
    Left = 321
    Top = 163
    Width = 122
    Height = 24
    DynProps = <>
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    Kind = dtkDateEh
    ParentFont = False
    TabOrder = 5
    Visible = True
  end
  object oLst_Ctes: TDBLookupComboboxEh
    Left = 73
    Top = 216
    Width = 287
    Height = 24
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'cte_id'
    ListField = 'cte_nombre_loc'
    ListSource = oDS_Ctes
    ParentFont = False
    TabOrder = 6
    Visible = True
  end
  object oAll_Ctes: TCheckBox
    Left = 366
    Top = 220
    Width = 134
    Height = 17
    Caption = 'Todos los clientes.'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    State = cbChecked
    TabOrder = 7
    OnClick = oAll_CtesClick
  end
  object oLst_Modelos: TDBLookupComboboxEh
    Left = 73
    Top = 284
    Width = 287
    Height = 24
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'jueg_cod'
    ListField = 'jueg_nombre'
    ListSource = oDS_Juegos
    ParentFont = False
    TabOrder = 8
    Visible = True
  end
  object oAll_Modelos: TCheckBox
    Left = 366
    Top = 288
    Width = 134
    Height = 17
    Caption = 'Todas los modelos.'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    State = cbChecked
    TabOrder = 9
    OnClick = oAll_ModelosClick
  end
  object oBtn_Print: TPngBitBtn
    Tag = 5
    Left = 198
    Top = 434
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Caption = 'Imprimir'
    ParentBiDiMode = False
    TabOrder = 13
    OnClick = oBtn_PrintClick
    PngImage.Data = {
      
      34790000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object oBtnExit: TPngBitBtn
    Tag = 8
    Left = 355
    Top = 434
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Cancel = True
    Caption = 'Salir'
    ParentBiDiMode = False
    TabOrder = 14
    OnClick = oBtnExitClick
    PngImage.Data = {
      
      BCF5F80FB9AC0245468DB03C0000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object oBtn_Preview: TPngBitBtn
    Tag = 5
    Left = 42
    Top = 434
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Caption = 'Visualizar'
    ParentBiDiMode = False
    TabOrder = 12
    OnClick = oBtn_PreviewClick
    PngImage.Data = {
      
      3F2EBDEDFF9CC1088CD0F0C7F60000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 499
    Width = 542
    Height = 19
    Panels = <
      item
        Width = 50
      end>
    ExplicitLeft = 8
    ExplicitTop = 389
    ExplicitWidth = 706
  end
  object oChapa: TEdit
    Left = 73
    Top = 353
    Width = 102
    Height = 21
    TabOrder = 10
  end
  object oAll_Chapas: TCheckBox
    Left = 185
    Top = 356
    Width = 97
    Height = 17
    Caption = 'Todas las Chapa:'
    Checked = True
    State = cbChecked
    TabOrder = 11
    OnClick = oAll_ChapasClick
  end
  object oOpt1: TRadioButton
    Tag = 1
    Left = 19
    Top = 71
    Width = 225
    Height = 17
    Caption = 'Informe general de m'#225'quinas'
    Checked = True
    TabOrder = 1
    TabStop = True
    OnClick = oOpt1Click
  end
  object oOpt2: TRadioButton
    Tag = 7
    Left = 250
    Top = 71
    Width = 225
    Height = 17
    Caption = 'Informe por rutas'
    TabOrder = 2
    OnClick = oOpt2Click
  end
  object oOpt3: TRadioButton
    Tag = 1
    Left = 19
    Top = 93
    Width = 225
    Height = 17
    Caption = 'Informe por proveedor de m'#225'quinas'
    TabOrder = 3
    OnClick = oOpt3Click
  end
  object oOpt4: TRadioButton
    Tag = 1
    Left = 250
    Top = 93
    Width = 225
    Height = 17
    Caption = 'Informe por cliente'
    TabOrder = 16
    OnClick = oOpt4Click
  end
  object oAll_Emp: TCheckBox
    Left = 88
    Top = 40
    Width = 286
    Height = 17
    Caption = 'Presentar datos de todas las empresas.'
    TabOrder = 17
    Visible = False
    OnClick = oAll_EmpClick
  end
  object oLst_Rutas: TDBLookupComboboxEh
    Left = 73
    Top = 319
    Width = 287
    Height = 24
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'rut_id'
    ListField = 'rut_nombre'
    ListSource = oDS_Rutas
    ParentFont = False
    TabOrder = 18
    Visible = False
  end
  object oAll_Routes: TCheckBox
    Left = 366
    Top = 323
    Width = 134
    Height = 17
    Caption = 'Todas las rutas.'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    State = cbChecked
    TabOrder = 19
    Visible = False
    OnClick = oAll_RoutesClick
  end
  object oCod_Maq: TEdit
    Left = 73
    Top = 388
    Width = 102
    Height = 21
    TabOrder = 20
  end
  object oAll_Maq: TCheckBox
    Left = 185
    Top = 391
    Width = 109
    Height = 17
    Caption = 'Todas las M'#225'quinas'
    Checked = True
    State = cbChecked
    TabOrder = 21
    OnClick = oAll_MaqClick
  end
  object oLst_Conses: TDBLookupComboboxEh
    Left = 72
    Top = 250
    Width = 287
    Height = 24
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'prov_cod'
    ListField = 'prov_nombre'
    ListSource = oDS_Maq_Prov
    ParentFont = False
    TabOrder = 22
    Visible = True
  end
  object oAll_Conses: TCheckBox
    Left = 366
    Top = 254
    Width = 168
    Height = 17
    Caption = 'Todos los Consesionarios'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    State = cbChecked
    TabOrder = 23
    OnClick = oAll_ConsesClick
  end
  object oDs_Emp: TDataSource
    AutoEdit = False
    DataSet = oQry_Empresa
    Left = 585
    Top = 136
  end
  object oQry_Empresa: TFDQuery
    Connection = oConection
    SQL.Strings = (
      'SELECT * FROM empresas where emp_inactivo=0')
    Left = 513
    Top = 136
  end
  object oConection: TFDConnection
    Params.Strings = (
      'Database=one2009_1'
      'User_Name=root'
      'CharacterSet=utf8'
      'Server=localhost'
      'DriverID=MySQL')
    LoginPrompt = False
    Left = 513
    Top = 11
  end
  object oDS_Ctes: TDataSource
    DataSet = oQry_Ctes
    Left = 585
    Top = 276
  end
  object oQry_Ctes: TFDQuery
    Connection = oConection
    SQL.Strings = (
      
        'SELECT ct.cte_id, UCASE(trim(ct.cte_nombre_loc)) AS cte_nombre_l' +
        'oc'
      'FROM clientes ct'
      'WHERE (ct.cte_inactivo=0)'
      'ORDER BY UCASE(trim(ct.cte_nombre_loc))  ')
    Left = 513
    Top = 276
  end
  object oDS_Juegos: TDataSource
    DataSet = oQry_Juegos
    Left = 585
    Top = 206
  end
  object oQry_Juegos: TFDQuery
    Connection = oConection
    SQL.Strings = (
      'SELECT jueg_cod, UCASE(trim(jueg_nombre)) AS  jueg_nombre'
      'FROM maquinas_juegos'
      'WHERE (jueg_inactivo=0)'
      'ORDER BY UCASE(trim(jueg_nombre));'
      '')
    Left = 513
    Top = 206
  end
  object oDS_Rutas: TDataSource
    DataSet = otRutas
    Left = 584
    Top = 337
  end
  object otRutas: TFDTable
    IndexName = 'rut_nombre'
    Connection = oConection
    UpdateOptions.UpdateTableName = 'rutas'
    TableName = 'rutas'
    Left = 512
    Top = 337
  end
  object oDS_Maq_Prov: TDataSource
    DataSet = otMaq_Prov
    Left = 592
    Top = 401
  end
  object otMaq_Prov: TFDTable
    IndexName = 'prov_nombre'
    Connection = oConection
    UpdateOptions.UpdateTableName = 'maquinas_prov'
    TableName = 'maquinas_prov'
    Left = 520
    Top = 401
  end
end
