object faplica_conta: Tfaplica_conta
  ...
  object oQry_Op: TFDQuery
    ...
    SQL.Strings = (
        ...
        'SELECT `id_op`, `op_emp_id`, `op_fecha`, `cte_id`, `cte_nombre_l' +
        'oc`, `op_tot_colect`, `op_tot_impjcj`, `op_tot_tec`, `op_tot_dev' +
        '`, `op_tot_otros`, `op_tot_cred`, `op_cal_cred`, `op_tot_sub`, `' +
        'op_tot_itbm`, `op_tot_tot`,`op_tot_brutoemp`, `op_tot_netoemp`'
      'FROM operacion op'
      
        'WHERE (op.op_aplica_interf =0) AND (op.cte_id="42") AND ((DATE_F' +
        'ORMAT(op.op_fecha, "%Y-%m-%d") >= "2018-02-13") AND (DATE_FORMAT' +
        '(op.op_fecha, "%Y-%m-%d") <= "2019-02-13"))'
