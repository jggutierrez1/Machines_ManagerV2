...
type
  ...
  private
    varlist: TStringlist;
    { contains variable names with integer values as "objects" }
    func: string; { the input expression }
    lasterror: string; { last error message }
    postfixlist: TStringlist;
    { list of expression terms in postfix format after parsing }
    ...
    { evaluate the expression after it has been determined to be vaild }
...
begin
  if varlist.addobject(uppercase(newvarname), TObject(newvarval)) >= 0 then
    result := true
  else
  begin
...
{ **************** GetNextToken ************ }
  ...
  Begin
    ...
    begin { get a variable or constant }
      i := 2;
      while (i <= length(s)) and not(s[i] in [' '] + opset) do
        inc(i);
      result := copy(s, 1, i - 1);
      delete(s, 1, i - 1);
    end;
...
{ *********** getInstackP ************ }
  ...
  begin
    i := 1;
    while (i <= opcount) and (s <> opstrings[i]) do
      inc(i);
    if i <= opcount then
      result := instackp[i]
    else
...
{ **************** ParensBalanced *********** }
  ...
  begin
    ...
    result := n = 0;
    if not result then
      showerror('Unbalanced parentheses in  expression' + #13 + origs);
  end;
...
begin { validequations }
  ...
  begin
    ...
    while t <> '' do
      ...
      begin
        ...
        if { check for adjacent operators }
          (prevop <> ' ') and (prevop <> '(') and (prevop <> ')') and
          (opobj.r.tokentype = operator) and (opobj.r.op <> lparen) and
          (opobj.r.op <> rparen) then
        begin
          showerror('Two adjacent operators found, "' + prevop + '" and "'
            + t + '"');
        ...
        if opobj.r.tokentype = operator then
          prevop := t
        else
          prevop := ' ';
        if (opobj.r.tokentype = variable) or (opobj.r.tokentype = constant) then
        begin
          if verbose then
            verboseList.add('___Add operand "' + t + '" to PostFix');
          addobject(t, opobj);
        end
        ...
        begin { close paren,')', found, unstack back to '(' }
          ...
          begin
            if verbose then
              verboseList.add('__Right paren found, unstack temp back to "("');
            ...
            begin
              ...
              dec(scount);
              if verbose then
                verboseList.add('______Pop "' + stackobj.id +
                  '" from temp and add to PostFix');
            end;
            TOpObj(astack[scount]).free; { free the lparen object }
            dec(scount);
            if verbose then
              verboseList.add('___Pop "(" from temp stack');
          end;
        ...
        begin { operator }
          while (scount > 0) and
            (getinstackp(astack[scount].id) >= getincomingp(t)) do
          begin
            if verbose then
              verboseList.add('___Push operator "' + t +
                '" to a temporary stack');
            y := astack[scount].id;
            stackobj := TOpObj(astack[scount]);
            if verbose then
              verboseList.add('___Operator "' + t +
                '" lower priority, pop higher priority temp stack item "' +
          ...
          if verbose then
            verboseList.add('___Push "' + t + '" to temp stack');
          inc(scount);
          astack[scount] := opobj;
        end;
        t := getnexttoken(s);
    ...
    begin
      stackobj := TOpObj(astack[scount]);
      if verbose then
        verboseList.add('___No more operators, pop temp stack item "' +
          astack[scount].id + '" and add to PostFix');
...
Begin
  ...
  scount := 0;
  if verbose then
    verboseList.add('Phase 2: Evaluate expression from PostFix List');
  with postfixlist do
    for i := 0 to count - 1 do
    begin
      ...
      begin
        if verbose then
          verboseList.add('___Retrieve next entry "' + id + '" from PostFix');
        if (r.tokentype in [variable, constant]) then
        Begin
          if verbose then
            verboseList.add('___Push "' + id + '" to temp stack');
          inc(scount);
          astack[scount] := opobj;
        end
        else
        begin { operation found }
          with astack[scount], r do
            ...
            begin
              ...
              if verbose then
                verboseList.add('___OpB = Pop variable ' + id + ' (value ' +
            ...
            begin
              ...
              if verbose then
              begin
                if id = intermediateresult then
                  verboseList.add('___OpB = Pop ' + id + ' (' + format('%.2f',
                    [opb]) + ') off of temp stack')
                else
                  verboseList.add('___OpB = Pop constant ' + id +
                    ' off of temp stack');
              end;
            end;
          dec(scount);
          ...
          begin
            ...
            begin
              ...
              if verbose then
                verboseList.add('___OpA = Pop variable ' + id + ' value ' +
                  inttostr(trunc(opa)) + ' off of temp stack');
            end
            else
            begin
              opa := Constantvalue;
              if verbose then
                verboseList.add('___OpA = Pop constant ' + id +
                  ' off of temp stack');
            end;
            ...
            case opobj.r.op of
              plus:
                Constantvalue := opa + opb;
              times:
                Constantvalue := opa * opb;
              minus:
                Constantvalue := opa - opb;
              divideby:
                Constantvalue := opa / { div } opb;
            end;
            if verbose then
              verboseList.add(format('___Calculate %.2f %s %.2f = %.2f and ' +
                'push result to temp stack', [opa, opobj.id, opb,
                Constantvalue]));
          end;
        end;
      end;
  ...
  begin { expression was a single variable or constant }
    with astack[1].r do
      if tokentype = variable then
        value := single(varlist.objects[variablenbr])
      else
        value := Constantvalue;
  end
  else if scount = 1 then
    value := astack[1].r.Constantvalue
  else
    showmessage('System error, too many operands');
  If verbose then
    verboseList.add
      (format('___Last added entry in temp stack is the result %6.2f',
      [value]));
end;
