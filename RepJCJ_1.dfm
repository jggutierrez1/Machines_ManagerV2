object FRepJCJ_1: TFRepJCJ_1
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Teportes de Producci'#243'n TIPO C.'
  ClientHeight = 541
  ClientWidth = 488
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Icon.Data = {
    
    0000000700000007000000FF000001FF0000C7FF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object olFecha1: TLabel
    Left = 34
    Top = 277
    Width = 34
    Height = 13
    Caption = 'Desde:'
  end
  object olFecha2: TLabel
    Left = 301
    Top = 277
    Width = 32
    Height = 13
    Caption = 'Hasta:'
  end
  object olCliente: TLabel
    Left = 18
    Top = 389
    Width = 37
    Height = 13
    Caption = 'Cliente:'
  end
  object olRuta: TLabel
    Left = 493
    Top = 259
    Width = 32
    Height = 13
    Caption = 'Rutas:'
    Visible = False
  end
  object olModelos: TLabel
    Left = 486
    Top = 286
    Width = 39
    Height = 13
    Caption = 'Modelos'
    Visible = False
  end
  object Shape1: TShape
    Left = 0
    Top = 439
    Width = 508
    Height = 2
    Pen.Color = clDefault
  end
  object Shape2: TShape
    Left = 0
    Top = 267
    Width = 508
    Height = 2
    Pen.Color = clDefault
  end
  object Shape3: TShape
    Left = 0
    Top = 380
    Width = 508
    Height = 2
    Pen.Color = clDefault
  end
  object Shape4: TShape
    Left = 0
    Top = 327
    Width = 508
    Height = 2
    Pen.Color = clDefault
  end
  object Label3: TLabel
    Left = 19
    Top = 16
    Width = 59
    Height = 16
    Caption = 'Empresa:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    Transparent = True
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 522
    Width = 488
    Height = 19
    Panels = <
      item
        Width = 50
      end>
  end
  object oOpt1: TRadioButton
    Tag = 1
    Left = 19
    Top = 63
    Width = 225
    Height = 17
    Caption = 'Informe Diario (Detallado Local)'
    Checked = True
    TabOrder = 1
    TabStop = True
    OnClick = oOpt1Click
  end
  object oOpt2: TRadioButton
    Tag = 2
    Left = 19
    Top = 92
    Width = 225
    Height = 17
    Caption = 'Informe Diario (Resumen Local)'
    TabOrder = 2
    OnClick = oOpt2Click
  end
  object oOpt3: TRadioButton
    Tag = 3
    Left = 19
    Top = 123
    Width = 225
    Height = 17
    Caption = 'Informe Diario (Detallado Fact.)'
    TabOrder = 3
    OnClick = oOpt3Click
  end
  object oOpt4: TRadioButton
    Tag = 7
    Left = 250
    Top = 63
    Width = 225
    Height = 17
    Caption = 'Informe J.C.J.'
    TabOrder = 4
    OnClick = oOpt4Click
  end
  object oOpt5: TRadioButton
    Tag = 8
    Left = 250
    Top = 92
    Width = 225
    Height = 17
    Caption = 'Informe por Local'
    TabOrder = 5
    OnClick = oOpt5Click
  end
  object oOpt6: TRadioButton
    Tag = 9
    Left = 250
    Top = 123
    Width = 225
    Height = 17
    Caption = 'Informe Por Modelo'
    TabOrder = 6
    OnClick = oOpt6Click
  end
  object oOpt7: TRadioButton
    Tag = 4
    Left = 19
    Top = 153
    Width = 225
    Height = 17
    Caption = 'Informe Producci'#243'n C/Metros Sencillo'
    TabOrder = 7
    OnClick = oOpt7Click
  end
  object oOpt9: TRadioButton
    Tag = 5
    Left = 19
    Top = 184
    Width = 225
    Height = 17
    Caption = 'Informe Resumido M'#225'quina/Local'
    TabOrder = 8
    OnClick = oOpt9Click
  end
  object oOpt10: TRadioButton
    Tag = 6
    Left = 19
    Top = 216
    Width = 225
    Height = 17
    Caption = 'Informe por Ruta'
    TabOrder = 9
    OnClick = oOpt10Click
  end
  object oBtn_Print: TPngBitBtn
    Tag = 5
    Left = 168
    Top = 459
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Caption = 'Imprimir'
    ParentBiDiMode = False
    TabOrder = 10
    OnClick = oBtn_PrintClick
    PngImage.Data = {
      
      34790000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object oBtnExit: TPngBitBtn
    Tag = 8
    Left = 331
    Top = 459
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Cancel = True
    Caption = 'Salir'
    ParentBiDiMode = False
    TabOrder = 11
    OnClick = oBtnExitClick
    PngImage.Data = {
      
      BCF5F80FB9AC0245468DB03C0000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object oBtn_Preview: TPngBitBtn
    Tag = 5
    Left = 18
    Top = 459
    Width = 144
    Height = 57
    Align = alCustom
    BiDiMode = bdLeftToRight
    Caption = 'Visualizar'
    ParentBiDiMode = False
    TabOrder = 12
    OnClick = oBtn_PreviewClick
    PngImage.Data = {
      
      3F2EBDEDFF9CC1088CD0F0C7F60000000049454E44AE426082}
    PngOptions = [pngBlendOnDisabled, pngGrayscaleOnDisabled]
  end
  object oFecha1: TDBDateTimeEditEh
    Left = 34
    Top = 296
    Width = 106
    Height = 21
    DynProps = <>
    EditButtons = <>
    Kind = dtkDateEh
    TabOrder = 13
    Visible = True
  end
  object oFecha2: TDBDateTimeEditEh
    Left = 301
    Top = 296
    Width = 106
    Height = 21
    DynProps = <>
    EditButtons = <>
    Kind = dtkDateEh
    TabOrder = 14
    Visible = True
  end
  object oChapa: TEdit
    Left = 134
    Top = 344
    Width = 102
    Height = 21
    Enabled = False
    TabOrder = 15
  end
  object oAll_Chapas: TCheckBox
    Left = 18
    Top = 346
    Width = 97
    Height = 17
    Caption = 'Todas las Chapa:'
    Checked = True
    State = cbChecked
    TabOrder = 16
    OnClick = oAll_ChapasClick
  end
  object oLst_Rutas: TDBLookupComboboxEh
    Left = 531
    Top = 256
    Width = 102
    Height = 21
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    KeyField = 'rut_id'
    ListField = 'rut_nombre'
    ListSource = oDS_Rutas
    TabOrder = 17
    Visible = False
  end
  object oLst_Ctes: TDBLookupComboboxEh
    Left = 18
    Top = 408
    Width = 254
    Height = 21
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    KeyField = 'cte_id'
    ListField = 'cte_nombre_loc'
    ListSource = oDS_Ctes
    TabOrder = 18
    Visible = True
  end
  object oAll_Ctes: TCheckBox
    Left = 278
    Top = 410
    Width = 127
    Height = 17
    Caption = 'Todos los clientes.'
    Checked = True
    State = cbChecked
    TabOrder = 19
    OnClick = oAll_CtesClick
  end
  object oAll_Routes: TCheckBox
    Left = 639
    Top = 260
    Width = 97
    Height = 17
    Caption = 'Todas las rutas.'
    Checked = True
    State = cbChecked
    TabOrder = 20
    Visible = False
    OnClick = oAll_RoutesClick
  end
  object oLst_Modelos: TDBLookupComboboxEh
    Left = 531
    Top = 283
    Width = 102
    Height = 21
    DynProps = <>
    DataField = ''
    Enabled = False
    EditButtons = <>
    KeyField = 'maqtc_modelo'
    ListField = 'maqtc_modelo'
    ListSource = oDS_Model
    TabOrder = 21
    Visible = False
  end
  object oAll_Modelos: TCheckBox
    Left = 639
    Top = 285
    Width = 97
    Height = 17
    Caption = 'Todas los modelos.'
    Checked = True
    State = cbChecked
    TabOrder = 22
    Visible = False
    OnClick = oAll_ModelosClick
  end
  object oLst_emp: TDBLookupComboboxEh
    Left = 84
    Top = 8
    Width = 373
    Height = 30
    DynProps = <>
    DataField = ''
    EditButtons = <>
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Arial'
    Font.Style = []
    KeyField = 'emp_id'
    ListField = 'emp_descripcion'
    ListSource = oDs_Emp
    ParentFont = False
    TabOrder = 23
    Visible = True
    OnCloseUp = oLst_empCloseUp
    OnExit = oLst_empExit
  end
  object oAll_Emp: TCheckBox
    Left = 88
    Top = 40
    Width = 286
    Height = 17
    Caption = 'Presentar datos de todas las empresas.'
    TabOrder = 24
    OnClick = oAll_EmpClick
  end
  object oOpt11: TRadioButton
    Tag = 10
    Left = 250
    Top = 153
    Width = 225
    Height = 17
    Caption = 'Informe Resumido M'#225'quina'
    TabOrder = 25
    OnClick = oOpt11Click
  end
  object oOpt12: TRadioButton
    Tag = 11
    Left = 250
    Top = 184
    Width = 225
    Height = 17
    Caption = 'Informe Gr'#225'fico Detalle [Local/M'#225'quinas]'
    TabOrder = 26
    OnClick = oOpt12Click
  end
  object oOpt13: TRadioButton
    Tag = 12
    Left = 250
    Top = 216
    Width = 225
    Height = 17
    Caption = 'Informe Gr'#225'fico Resumen [Local]'
    TabOrder = 27
    OnClick = oOpt13Click
  end
  object oOpt14: TRadioButton
    Tag = 14
    Left = 250
    Top = 244
    Width = 225
    Height = 17
    Caption = 'Informe J.C.J. Detalle'
    TabOrder = 28
    OnClick = oOpt14Click
  end
  object oOpt15: TRadioButton
    Tag = 15
    Left = 19
    Top = 244
    Width = 225
    Height = 17
    Caption = 'Informe NETWIN'
    TabOrder = 29
    OnClick = oOpt15Click
  end
  object oDS_Rutas: TDataSource
    DataSet = otRutas
    Left = 488
    Top = 369
  end
  object oDS_Ctes: TDataSource
    DataSet = oQry_Ctes
    Left = 488
    Top = 428
  end
  object oDS_Model: TDataSource
    DataSet = oQry_Modelos
    Left = 488
    Top = 488
  end
  object oDs_Emp: TDataSource
    AutoEdit = False
    DataSet = oQry_Empresa
    Left = 488
    Top = 310
  end
  object oConection: TFDConnection
    Params.Strings = (
      'Database=one2009_1'
      'User_Name=root'
      'CharacterSet=utf8'
      'Server=localhost'
      'DriverID=MySQL')
    LoginPrompt = False
    Left = 664
    Top = 299
  end
  object oQry_Empresa: TFDQuery
    Connection = oConection
    SQL.Strings = (
      'SELECT * FROM empresas where emp_inactivo=0')
    Left = 544
    Top = 310
  end
  object otRutas: TFDTable
    IndexName = 'rut_nombre'
    Connection = oConection
    UpdateOptions.UpdateTableName = 'rutas'
    TableName = 'rutas'
    Left = 544
    Top = 369
  end
  object oQry_Modelos: TFDQuery
    Connection = oConection
    SQL.Strings = (
      'SELECT '
      #9'maqtc_modelo '
      'FROM maquinastc '
      'GROUP BY maqtc_modelo '
      'ORDER BY maqtc_modelo;')
    Left = 544
    Top = 488
  end
  object oQry_Ctes: TFDQuery
    Connection = oConection
    SQL.Strings = (
      
        'SELECT ct.* FROM clientes ct WHERE ct.cte_inactivo=0 ORDER BY ct' +
        '.cte_nombre_loc ')
    Left = 544
    Top = 424
  end
end
